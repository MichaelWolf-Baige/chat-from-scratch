...
| 模型参数 | 13.8M |
| 训练 token | 131.1M |
| 全局 batch size | 128（4×32） |
| 总步数 | 500 |
| Warmup | 50 步 |
| LR schedule | Cosine decay: 3e-4→1e-6 |

**问题的数学本质**：

Cosine decay 的公理——前 50 步 warmup 到峰值 1.2e-3（因为 linear scaling×4），然后立刻开始衰减：

```
有效学习 ≈ (峰值区间的步数) × (峰值 LR)
           ≈ (step 50~150, 约 100 步) × (~1e-3)
```

也就是说模型只有 ~100 步在高学习率区间，占总步数的 20%。其余 80% 的步数 LR 太低，loss 不再变化。

**三条修复路径**（待讨论）：

| 方案 | 改动 | 预期 PPL | 复杂度 |
|------|------|---------|--------|
| **A** | Constant LR（去掉 cosine decay） | 降低（不确定多少） | 低 |
| **B** | A + 训练到 280M tokens | 100-300 | 中 |
| **C** | B + 加入英文数据 | 30-60 | 高 |
